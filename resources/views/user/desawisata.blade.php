                ...
                <div class="row justify-content-center">
                    @foreach ($desaWisata as $desa)
                        <div class="col-md-4 mb-4">
                            <div class="text-center p-3 border rounded shadow-sm bg-white equal-height">
                                <img src="{{ $desa->gambar ? asset('storage/' . $desa->gambar) : 'path/to/default-image.jpg' }}"
                                    class="img-fluid rounded mb-3 fixed-image" alt="Gambar Desa Wisata">
                                <h5 class="fw-bold">{{ $desa->nama }}</h5>
                                <p class="text-justify text-muted">{{ $desa->deskripsi }}</p>
                            </div>
                        </div>
                    @endforeach
                </div>
